...
const DEFAULT_USD_RATE = 31.5; // 美元兌新台幣參考匯率，可在設定裡調整
const TICK_MS = 4000;
...
export default function App() {
    ...
    (async () => {
      ...
      try {
        ...
        if (r2 && r2.value) {
          ...
          api = s.apiBaseUrl || "";
          usd = s.usdRate ?? DEFAULT_USD_RATE;
        }
      } catch (e) { /* 尚無資料，使用預設 */ }
      try {
  ...
  const persistSettings = useCallback(async (th, api, usd) => {
    try { await storageSet("settings", JSON.stringify({ threshold: th, apiBaseUrl: api, usdRate: usd })); } catch (e) { /* 略過儲存失敗 */ }
  }, []);
  ...
  useEffect(() => {
    ...
    async function fetchFxRate() {
      try {
        const cleanBase = apiBaseUrl.trim().replace(/\/$/, "");
        const res = await fetch(`${cleanBase}/api/fxrate`);
        if (!res.ok) return;
        const data = await res.json();
        if (typeof data.rate === "number" && !Number.isNaN(data.rate)) {
          setUsdRate(data.rate);
          persistSettings(thresholdRef.current, apiBaseUrl, data.rate);
        }
      } catch (e) {
        // 抓不到匯率就維持原本的數字，不影響其他功能
      }
    }

    fetchFxRate();
    const fxInterval = setInterval(fetchFxRate, 60 * 60 * 1000); // 匯率變動不大，每小時檢查一次即可
  ...
  const rows = holdings.map((h) => {
    ...
    const fx = h.market === "US" ? usdRate : 1; // 美股先依匯率換算成新台幣，總資產才會正確加總
    const valueTWD = value * fx;
    const costTWD = costValue * fx;
    return { ...h, live: l, value, gain, gainPct, dayChangePct, isAlert, valueTWD, costTWD };
  });
  ...
  const marketRows = rows.filter((r) => {
    if ((r.market || "TW") !== assetMarketTab) return false;
    if (selectedBankFilter && (r.bank || UNCATEGORIZED) !== selectedBankFilter) return false;
    return true;
  });
  ...
  function saveThreshold(v) {
    setThreshold(v);
    persistSettings(v, apiBaseUrl, usdRate);
  }
  function saveApiBaseUrl(v) {
    const clean = v.trim();
    setApiBaseUrl(clean);
    persistSettings(threshold, clean, usdRate);
  }
  function saveUsdRate(v) {
    setUsdRate(v);
    persistSettings(threshold, apiBaseUrl, v);
  }
  ...
  return (
    <div className="pw-app">
      <style>{`
        ...
        .pw-market-breakdown{ margin-top:4px; font-size:12px; color:var(--muted); font-family:'IBM Plex Mono',monospace; }
        ...
        .pw-bank-right{ display:flex; flex-direction:column; align-items:flex-end; flex:none; }
        .pw-bank-value{ font-family:'IBM Plex Mono',monospace; font-size:13px; font-weight:400; color:var(--ink); line-height:1.4; white-space:nowrap; }
        .pw-bank-gain{ font-family:'IBM Plex Mono',monospace; font-size:13px; margin-top:2px; line-height:1.4; white-space:nowrap; }
      ...
      {page === "assets" && (
        <>
          ...
          <div className="pw-card pw-overview-card">
            ...
            {heroView === "overview" && (
              <div>
                ...
                <p className="pw-market-breakdown">台股 {money(twTotalValue)} · 美股 {money(usTotalValue)}</p>
          ...
          <div className="pw-section-title-row">
            ...
            {selectedBankFilter && (
              <button className="pw-row-icon" onClick={() => setSelectedBankFilter(null)}>
                清除篩選（目前只看「{selectedBankFilter}」）
              </button>
            )}
          </div>
          <div className="pw-bank-cards">
            {bankRows.map((b) => (
              <div
                className={"pw-bank-card" + (selectedBankFilter === b.bank ? " selected" : "")}
                key={b.bank}
                onClick={() => setSelectedBankFilter(selectedBankFilter === b.bank ? null : b.bank)}
              >
          ...
          <div className="pw-table-wrap">
            {marketRows.length === 0 ? (
              <div className="pw-empty">
                {selectedBankFilter
                  ? `「${selectedBankFilter}」在${assetMarketTab === "TW" ? "台股" : "美股"}沒有持股。`
                  : `${assetMarketTab === "TW" ? "尚無台股倉位" : "尚無美股倉位"}，點右上角「新增倉位」開始追蹤。`}
              </div>
            ) : (
              <table className="pw-table">
                <thead>
                  <tr>
                    ...
                    <th>成本金額</th><th>市值</th><th>損益</th><th></th>
                  </tr>
                </thead>
                <tbody>
                  {marketRows.map((row) => (
                    <tr key={row.id} className={row.isAlert ? "alert-row" : ""}>
                      ...
                      <td>{moneyFor(row.cost * row.qty, row.market)}</td>
          ...
          {showSettings && (
            <div className="pw-overlay" onClick={() => setShowSettings(false)}>
              <div className="pw-modal" onClick={(e) => e.stopPropagation()}>
                ...
                <div className="pw-field">
                  <label>美元兌新台幣匯率{apiBaseUrl ? "（已自動抓取，可手動覆蓋）" : "（美股會依此換算進總資產）"}</label>
                  <input type="number" step="0.01" value={usdRate} onChange={(e) => saveUsdRate(parseFloat(e.target.value) || DEFAULT_USD_RATE)} />
                </div>
                <p className="pw-hint">
                  {quoteMode === "live"
                    ? "目前已成功連線後端，顯示的是真實報價。"
                  ...
                  {apiBaseUrl
                    ? "已填後端網址，會自動每小時向後端查詢一次最新匯率，不用手動更新；若後端一時查不到匯率，會維持上一次抓到的數字。"
                    : "目前沒有後端網址可以自動查匯率，請手動填寫參考值。"}
